...
function parseClassProJson(data) {
  ...
  return {
    ...
    alertConfig:      parse('sc-alert-config') || {},
    ...
    vacances:         parse('cdc-vacances')    || [],
    bulletins:        parse('cdc-data')        || [],
    reunions:         parse('cdc-reunions')    || [],
    rencontres:       parse('cdc-rencontres')  || [],
    dashNotes:        parse('dash-notes')      || [],
    dashTaches:       parse('dash-taches')     || [],
    dashTravailRapide: parse('dash-travail-rapide') || [],
    _raw: data,
  };
}
...
function fmtDate(isoStr) {
  if (!isoStr) return '—';
  const d = isoStr && isoStr.length === 10 ? new Date(isoStr + 'T12:00') : new Date(isoStr);
